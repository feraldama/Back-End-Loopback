...
export class PaisController {
  ...
  async count(@param.where(Pais) where?: Where<Pais>): Promise<Count> {
  ...
  async find(@param.filter(Pais) filter?: Filter<Pais>): Promise<Pais[]> {
  ...
  async findById(
    ...
    @param.filter(Pais, { exclude: "where" })
    filter?: FilterExcludingWhere<Pais>
  ): Promise<Pais> {
